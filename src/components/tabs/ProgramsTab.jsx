    ...
    <div style={{ padding: '24px 20px', paddingBottom: 'calc(var(--nav-h) + 24px)', display: 'flex', flexDirection: 'column', gap: 24, overflowY: 'auto', height: '100%', boxSizing: 'border-box' }}>
      ...
      <div className="stagger-item">
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 4 }}>
          ...
          {/* Explore presets button */}
          <button onClick={() => setBrowserOpen(true)} className="pressable" style={{
                  ...
                  <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 8 }}>
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4, flexWrap: 'wrap' }}>
                        ...
                        {isRecommendedPreset && (
                          <span style={{ display: 'inline-flex', alignItems: 'center', gap: 3, fontSize: 10, fontWeight: 700, padding: '2px 8px', borderRadius: 'var(--r-pill)', background: 'rgba(212,168,67,0.12)', color: '#D4A843', border: '1px solid rgba(212,168,67,0.3)' }}>
                            <Sparkles size={8} /> Para ti
                          </span>
                        )}
                        {!program.presetId && (
                          <span style={{ fontSize: 10, fontWeight: 700, padding: '2px 8px', borderRadius: 'var(--r-pill)', background: 'var(--surface3)', color: 'var(--text3)', border: '1px solid var(--border)' }}>
                            Personalizado
                          </span>
                        )}
                      </div>
                      {program.description && (
                        <p style={{ fontSize: 13, color: 'var(--text2)', lineHeight: 1.4, marginBottom: 4 }}>{program.description}</p>
                      )}
                      <p style={{ fontSize: 12, color: 'var(--text3)' }}>
                        {(program.days || []).length} días{program.weeks ? ` · ${program.weeks} semanas` : ''}
                      </p>
                    </div>
                    <div style={{ display: 'flex', gap: 6, flexShrink: 0, marginLeft: 12 }}>
                      <button onClick={() => openProgramEditor(program)} className="pressable" style={{ width: 34, height: 34, borderRadius: 10, background: 'var(--surface3)', border: '1px solid var(--border)', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer' }}>
                        <Edit2 size={14} color="var(--text2)" />
                      </button>
                      <button onClick={() => deleteProgram(program.id)} className="pressable" style={{ width: 34, height: 34, borderRadius: 10, background: 'var(--red-dim)', border: '1px solid rgba(229,83,75,0.2)', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer' }}>
            ...
            {templates.map((template) => {
              ...
              return (
                <div key={template.id} style={{
                  ...
                  <button onClick={() => openTemplateEditor(template)} className="pressable" style={{ width: 34, height: 34, borderRadius: 10, background: 'var(--surface3)', border: '1px solid var(--border)', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', flexShrink: 0 }}>
                    <Edit2 size={14} color="var(--text2)" />
                  </button>
                  <button onClick={() => deleteTemplate(template.id)} className="pressable" style={{ width: 34, height: 34, borderRadius: 10, background: 'var(--red-dim)', border: '1px solid rgba(229,83,75,0.2)', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', flexShrink: 0 }}>
                    <Trash2 size={14} color="var(--red)" />
                  </button>
                </div>
              )
      ...
      <ProgramBrowser open={browserOpen} onClose={() => setBrowserOpen(false)} onProgramActivated={() => onSwitchTab?.('today')} />
    </div>
  )
}
